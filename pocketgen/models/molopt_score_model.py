...
def get_refine_net(refine_net_type, config):
    ...
    elif refine_net_type == 'graphormer':
        refine_net = Graphormer(
            n_layers=config.num_layers,
            num_heads=config.n_heads,
            hidden_dim=config.hidden_dim,
            dropout_rate=config.dropout_rate,
            intput_dropout_rate=config.input_dropout_rate,
            ...
            flag=config.flag,
            flag_m=config.flag_m,
            flag_step_size=config.flag_step_size,
            flag_mag=config.flag_mag
        )
    else:
...
class ScorePosNet3D(nn.Module):
    ...
    def get_diffusion_loss(
        ...
        num_graphs = batch_protein.max().item() + 1 # batch_num
