...
import utils.transforms_prop as trans_prop
...
if __name__ == '__main__':
    ...
    # Load configs
    config = misc.load_config(args.config) # EasyDict, read from yaml file
    config_name = os.path.basename(args.config)[:os.path.basename(args.config).rfind('.')] # training.yml -> training
    ...
    if config.model.model_type == 'graphormer':
        transform_list.append(trans_prop.LigandCountNeighbors())
        transform_list.append(trans_prop.ligand_dismat())
    ...
    train_iterator = utils_train.inf_iterator(DataLoader(
        ...
        collate_fn=my_collate # add collate_fn
    ))

    val_loader = DataLoader(val_set, config.train.batch_size, shuffle=False,
                            follow_batch=FOLLOW_BATCH, exclude_keys=collate_exclude_keys, collate_fn=my_collate)
