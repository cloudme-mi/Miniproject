...
class FeaturizeProteinAtom(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        amino_acid = F.one_hot(data.protein_atom_to_aa_type, num_classes=self.max_num_aa) # (N_atoms, N_amino_acids)
        is_backbone = data.protein_is_backbone.view(-1, 1).long() # (N_atoms, 1)
        x = torch.cat([element, amino_acid, is_backbone], dim=-1) # (N_atoms, N_elements + N_amino_acids + 1)
