...
class LigandCountNeighbors(object):
    ...
    def __call__(self, data):
        data.ligand_num_neighbors = self.count_neighbors( # as the degree of the atom
...
class ligand_dismat(object):
    def __init__(self):
        super(ligand_dismat, self).__init__()

    def __call__(self, data):
        pos = torch.cat([data.ligand_pos, data.ligand_pos], dim=0)
        num_nodes = pos.size(0)
        pos = pos.view(num_nodes, 1, 3)
        pos = pos.repeat(1, num_nodes, 1)
        pos_t = pos.permute(1, 0, 2)
        dist = torch.norm(pos - pos_t, dim=2)
        data.liagand_dismat = dist
        return data
